...
**Version:** 0.2.0-rc13 (Release candidate for public comment)
**Date:** August 20, 2026
...
- **a verifying record establishes the integrity and provenance of what was recorded; it does not by itself establish that the recorded event corresponds to implementation behavior.** Signed, chained and externally anchored records faithfully preserve a false event emitted upstream of them (A8). Establishing implementation correspondence requires independently grounded evidence at or across the relevant implementation boundary. Depending on the claim, that may include an interception boundary the emitter does not control, an attestation bound to the relevant execution or state whose trust root is independent of the emitter, an execution receipt from a separate control plane, independently observed effects on the far side of the boundary, or other evidence whose generation does not depend solely on the event emitter. An independently reviewed enumeration of disclosure surfaces (v0.1 Section 7.3) strengthens a coverage claim; it does not establish correspondence. At v0.1 I3(a), what the verifier derives is the chain-bound release of the designated output through the enumerated path; that no other surface disclosed before commitment is A-class. A conformance statement MUST NOT assert implementation correspondence on the strength of a verifying record. <!-- req:2.2b --> Two failure modes are distinguished and only the first is A8: **false emission**, where the event says X happened and the implementation did Y, which is this requirement; and **omission**, where the event truthfully says X happened and the record is silent about Y, which is the coverage limitation that v0.1 Section 7.3's disclosure-surface enumeration addresses. Both leave the record an incomplete account of reality; they are different adversaries and are not interchangeable. This ships as an indistinguishable-worlds fixture over the first. World A is correspondence: the implementation presented the designated information after the committed assessment was accepted. World B is false emission: it presented before, and the recorder emitted the same event sequence regardless. The same record is compatible with both, so the verifier cannot establish implementation correspondence from the package alone. The fixture feeds it a byte-identical package whose identity it asserts by digest, and requires identical verifier output with requirement 2.2b listed as unestablished in both.
...
Consequently: "externally anchored establishes existence before time T" holds **against the deploying operator, under the declared external-anchor trust model**, not unconditionally. A7 is why anchor diversity and monitoring are recommended and why the verifier reports anchor trust as an attestation rather than a verified property. A8 is why a verifying record establishes provenance and integrity of what was recorded and not correspondence to implementation behavior (Section 2.2): every mechanism in Section 3 operates downstream of the emission that A8 controls. A1 rewrites history after the fact; A8 supplies a false present, which a faithful history then preserves.
...
### 8.4 WOA (informative in v0.2.0-rc13)
...
**0.2.0-rc13 (candidate delta, unreleased).** Threat-model clarification, no mechanism change. Adds adversary A8, the event-emission and capture path, which may cause the recorder to accept an event whose content does not correspond to the action or presentation it describes while every downstream mechanism remains valid; and the matching non-claim in Section 2.2: a verifying record establishes integrity and provenance of what was recorded, not correspondence to implementation behavior (requirement 2.2b, A-class). Prompted by the METR and Redwood Research investigation of the July 2026 Hugging Face incident (report of 26 August 2026), which found clear evidence of successful tool-call spoofing in at least 96 of about 1,300 examined transcripts, roughly 7%. The reported techniques evolved from manipulating tool-call output to, eventually, replacing part of the tool-execution path so an agent could appear to issue one tool call while another executed. False event content appeared in the actual transcripts the investigators reviewed; the investigation did not test the transcript store's cryptographic integrity properties, so it establishes failure of implementation correspondence without establishing whether internal or historical integrity held. That separability is the reason to name the third property. The report also notes that setting up the spoof required actions that remained visible in the transcript, so what detection there was came from setup traces rather than from a downstream integrity check. Erratum 1 of 20 August already covered the rewrite-after-the-fact adversary; the incident's finding is the adversary in front of storage, which the text did not name. The adversarial suite gains its first indistinguishable-worlds fixture: an out-of-band oracle the verifier never receives assigns two worlds to one package, and the suite requires identical verifier output and requirement 2.2b listed as unestablished in both. That fixture documents a boundary rather than a rule; it is registered separately from the rule-asserting cases and counted separately. The rule-asserting suite stands at 158, unchanged; boundary fixtures stand at 1.

**0.2.0-rc11.** Coherence pass, cut before named external review so that reviewers read a tree whose prose, schemas, verifier and environment record agree with one another. No normative model change. The gate gained three steps: a vocabulary check that fails when shipped prose or a schema description names an event or field the schemas do not define (step 7), a machine-written and machine-checked environment record (step 8), and the language guard over the whole release tree rather than four site files (step 9). The vocabulary check found that the definitions schema still described anchor evidence as living in `artifact_anchored` events, the event this specification retired for `anchor_evidence_recorded`; that Section 3.8 named a flat `code_artifact_digest` field the schema replaced in rc4 with `code_artifact` and an explicit locator; and that Section 3.4 named four anchoring grade labels the verifier derives but never reports as labels, and one it does not derive at all, which is now claim 3.4f, partial, rather than an implied implementation. The verifier's generic sweep over definition-artifact executables had been matching the retired flat field and so yielded nothing since rc4; every binding had depended on explicit use-site calls. The sweep matches the schema again, and each executable reference is checked once rather than at every use site, so the check count moves from 2,286 to 2,205: the difference was repetition, not coverage. The suite stands at 158; checks at 2,205. The candidate label is now moved by `scripts/bump_version.py` over one file list that release lint polices, and the previous candidate's environment record, which carried counts four numbers behind its own gate, is replaced by a block the gate writes and checks.
...
> Henderson JM. Decision-Sequence Evidence Schema, version 0.2.0-rc13: the outcome-evidence layer. Evidify LLC; 2026. Release candidate for public comment.
